...
<form method="POST" action="{{ isset($task) ? route('tasks.update', $task) : route('tasks.store') }}">
    ...
    <input type="text" name="title" class="form-control mb-2" placeholder="Title" value="{{ old('title', $task->title ?? '') }}">
    ...
    @endphp
    <input type="datetime-local" name="due_date" class="form-control mb-2" value="{{ old('due_date', $dueDateFormatted) }}">
